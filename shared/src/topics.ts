...
export const TOPIC_DEFINITIONS: Array<Pick<Topic, 'name' | 'description'> & { keywords: string[] }> = [
...
export type TopicWithKeywords = Topic & { keywords: string[] };

export const TOPICS: TopicWithKeywords[] = TOPIC_DEFINITIONS.map((t) => ({
  id: undefined,
  name: t.name,
  slug: slugify(t.name),
